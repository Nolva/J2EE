...
public class EmployeeController {
    ...
    public ModelAndView show(String page, Model model){
            for (Employee employee : employeeService.ListEmployee()){
            System.out.println(employee.getEmployeeId());
            System.out.println(employee.getEmpName());
        }
        List<Employee> employeeList = employeeService.ListEmployee();
        model.addAttribute("totalCount", employeeList.size());
        //总页数
        int totalPage;
        if(employeeList.size()%CUSTOMER_PAGE_SIZE == 0) {
            totalPage = employeeList.size()/CUSTOMER_PAGE_SIZE;
        }else{
            totalPage = employeeList.size()/CUSTOMER_PAGE_SIZE + 1;
        }
        ...
        employeeList = employeeService.ListManagerByPage(startRow, CUSTOMER_PAGE_SIZE);
